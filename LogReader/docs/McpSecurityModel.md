...
|---|---|---|
| MCP client to stdio process | The user trusts the configured client to start WeezTail and receive selected excerpts. | Exact absolute command plus sole `--mcp-stdio`; protocol-only stdout; sanitized stderr; no shell, HTTP, token, or UI-control surface. |
...
| Packaging and upgrade | The installed binary is executable code and may be locked by a client. | Same reviewed executable as the UI; no service or daemon; published stdio smoke test; stop client processes before replacement. |
...
5. A running MCP process can hold `WeezTail.exe` open until its client closes stdin or terminates it.
