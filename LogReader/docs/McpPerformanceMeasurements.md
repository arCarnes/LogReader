...
Measurement report schema version 4 also exhausts signed search cursors when the configured file count exceeds the 50-file page size and exercises one-call `count_logs` over the same candidate set. It records the selected synthetic-fixture query, cursor page count/coverage, returned and omitted-zero-hit file-record counts, count completion, bucket density, cumulative serialized response bytes, selected/searched/skipped/failed/remaining/matched counts, returned/matching-line/occurrence counts, incomplete reasons, traversal bytes/files, elapsed scan time, and peak disk/UNC gate concurrency. It contains no configured paths or returned log text.
