...
public class SearchServiceTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task TrySearchFileIndexedAsync_SparseUnsortedScope_MatchesSequentialResultsInLineOrder()
    {
        var lines = Enumerable.Range(1, 200)
            .Select(line => line is 3 or 4 or 80 ? $"hit {line}" : $"skip {line}")
            .ToArray();
        var path = await CreateTestFile("indexed-sparse.log", string.Join('\n', lines));
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
            LineScopesByFilePath = new Dictionary<string, SearchLineScope>(StringComparer.OrdinalIgnoreCase)
            {
                [path] = new()
                {
                    ...
                    LineNumbers = new[] { 80, 4, 3, 4, -1, 201 }
                }
            }
        };
        var reads = new List<(int StartLine, int Count)>();

        var indexed = await _searchService.TrySearchFileIndexedAsync(
            path,
            request,
            FileEncoding.Utf8,
            lines.Length,
            (startLine, count, _, _) =>
            {
                reads.Add((startLine, count));
                return Task.FromResult<IReadOnlyList<string>>(lines.Skip(startLine).Take(count).ToArray());
            });
        var sequential = await _searchService.SearchFileAsync(path, request, FileEncoding.Utf8);

        Assert.NotNull(indexed);
        Assert.Equal(new[] { (2, 2), (79, 1) }, reads);
        Assert.Equal(
            sequential.Hits.Select(hit => (hit.LineNumber, hit.LineText, hit.MatchStart, hit.MatchLength)),
            indexed.Hits.Select(hit => (hit.LineNumber, hit.LineText, hit.MatchStart, hit.MatchLength)));
    }

    [Fact]
    public async Task TrySearchFileIndexedAsync_DenseIncludeScope_RequestsSequentialFallback()
    {
        var path = await CreateTestFile("indexed-dense.log", "hit\n");
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
            LineScopesByFilePath = new Dictionary<string, SearchLineScope>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                {
                    Mode = SearchLineScopeMode.IncludeOnly,
                    LineNumbers = Enumerable.Range(1, 3).ToArray()
                }
            }
        };
        ...
        var result = await _searchService.TrySearchFileIndexedAsync(
            ...
            (_, _, _, _) => throw new InvalidOperationException("Dense scopes must not use indexed reads."));

        Assert.Null(result);
    }

    [Fact]
    public async Task TrySearchFileIndexedAsync_ExcludeScope_RequestsSequentialFallback()
    {
        var path = await CreateTestFile("indexed-exclude.log", "hit\n");
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
        ...
        var result = await _searchService.TrySearchFileIndexedAsync(
            ...
            FileEncoding.Utf8,
            indexedLineCount: 100,
            (_, _, _, _) => throw new InvalidOperationException("Exclude scopes must not use indexed reads."));

        Assert.Null(result);
    }

    [Fact]
    public async Task TrySearchFileIndexedAsync_ChangedIndexRead_RequestsSequentialFallback()
    {
        var path = await CreateTestFile("indexed-stale.log", "hit\n");
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
        ...
        var result = await _searchService.TrySearchFileIndexedAsync(
            ...
            FileEncoding.Utf8,
            indexedLineCount: 100,
            (_, _, _, _) => Task.FromResult<IReadOnlyList<string>>(new[] { "hit" }));

        Assert.Null(result);
    }

    [Fact]
    public async Task TrySearchFileIndexedAsync_EmptyOrOutOfRangeScope_ReturnsEmptyWithoutReading()
    {
        var path = await CreateTestFile("indexed-empty.log", "hit\n");
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
        ...
        var result = await _searchService.TrySearchFileIndexedAsync(
            ...
            FileEncoding.Utf8,
            indexedLineCount: 100,
            (_, _, _, _) => throw new InvalidOperationException("Empty scopes must not read the index."));
    ...
    [Fact]
    public async Task TrySearchFileIndexedAsync_HighlyFragmentedScope_RequestsSequentialFallback()
    {
        var path = await CreateTestFile("indexed-fragmented.log", "hit\n");
        var request = new SearchRequest
        {
            Query = "hit",
            FilePaths = new List<string> { path },
            LineScopesByFilePath = new Dictionary<string, SearchLineScope>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                {
                    Mode = SearchLineScopeMode.IncludeOnly,
                    LineNumbers = Enumerable.Range(0, SearchService.IndexedSearchMaximumBatchCount + 1)
                        .Select(index => index * 100 + 1)
                        .ToArray()
                }
            }
        };
        ...
        var result = await _searchService.TrySearchFileIndexedAsync(
            ...
            request,
            FileEncoding.Utf8,
            indexedLineCount: 10_000,
            (_, _, _, _) => throw new InvalidOperationException("Fragmented scopes must not use indexed reads."));

        Assert.Null(result);
    }

    [Fact]
    public async Task SearchFilesAsync_IndexedAttemptUnavailable_FallsBackToSequentialScan()
    {
        var path = await CreateTestFile("indexed-fallback.log", "skip\nhit\n");
        var request = new SearchRequest { Query = "hit", FilePaths = new List<string> { path } };
        var indexedAttempts = 0;

        var results = await _searchService.SearchFilesAsync(
            request,
            new Dictionary<string, FileEncoding> { [path] = FileEncoding.Utf8 },
            (_, _, _, _) =>
            {
                indexedAttempts++;
        ...
        Assert.Equal(1, indexedAttempts);
        Assert.Single(results);
        Assert.Single(results[0].Hits);
        Assert.Equal(2, results[0].Hits[0].LineNumber);
    }
