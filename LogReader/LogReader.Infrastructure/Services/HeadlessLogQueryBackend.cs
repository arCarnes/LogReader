...
public sealed partial class HeadlessLogQueryBackend : ILogQueryBackend
{
        ...
        CancellationToken ct)
    {
        ...
        ArgumentNullException.ThrowIfNull(request);
        var requestId = CreateRequestId();
        var validation = ValidateSearchRequest(
            request,
            out var effectiveFileLimit,
            out var effectiveHitsPerFile,
            out var effectiveTotalHits,
            isWql);
        ...
        CancellationToken ct)
    {
        ...
        for (var index = 0; index < selection.Files.Length; index++)
        {
            ...
            if (wqlParsing?.IsTruncated == true)
                truncationReasons.Add("field_metadata_limit");
            ...
            prepared.WqlParsing = wqlParsing;
            ...
            if (!string.IsNullOrWhiteSpace(raw.Error))
            {
                ...
                prepared.Error = raw.FieldExtractionTimedOut
                ...
                prepared.FileTruncated = retainedProvenance.IsTruncated || wqlParsing?.IsTruncated == true;
                prepared.IncompleteReasons.Add(failureReason);
                continue;
            }
        ...
        foreach (var prepared in preparedFiles)
        {
            ...
            if (prepared.Hits.Any(static hit => hit.WqlFieldsTruncated))
            {
                prepared.FileTruncated = true;
                truncationReasons.Add("field_output_limit");
                prepared.IncompleteReasons.Add("response_truncated");
            }
            if (prepared.WqlParsing?.IsTruncated == true)
            {
                prepared.FileTruncated = true;
                truncationReasons.Add("field_metadata_limit");
                prepared.IncompleteReasons.Add("response_truncated");
            ...
            {
                WqlParsing = prepared.WqlParsing,
            ...
            if (prepared.Hits.Count > 0 ||
                ...
                prepared.FileTruncated ||
                prepared.WqlParsing is not null ||
        ...
        ResponseCharacterBudget budget)
    {
        ...
        foreach (var hit in hits)
        {
            ...
            mappedHits.Add(AttachWqlFields(new LogSearchHit(
                hit.LineNumber,
                retained.MatchStart,
                retained.MatchLength), hit, budget));
    ...
    private sealed class PreparedSearchFile
    {
        ...
        public LogWqlParsingStatistics? WqlParsing { get; set; }
